...
def test_mobilenet():
    train_ds, val_ds, class_names = data_load("F:/datas/tmp/data/tttt/trash_jpg", 224, 224, 4)  #加载训练集、数据集
    print(class_names)  #打印类别名称
    model = tf.keras.models.load_model("models/mobilenet_245_epoch30.h5")   #加载模型
    model.summary()
    loss, accuracy = model.evaluate(val_ds) #返回损失和准确率
    print('Mobilenet test accuracy :', accuracy)

#测试CNN模型

def test_cnn():
    train_ds, val_ds, class_names = data_load("F:/datas/tmp/data/tttt/trash_jpg", 224, 224, 4)
...
def draw_heatmap(folder_name):
    ...
    #遍历所有图像路径，对每个图像进行处理和分类
    model = tf.keras.models.load_model("models/mobilenet_245_epoch30.h5")   #加载预训练的模型
...
def load_pickle(filename="pickle_result.pickle"):
    f = open(filename, 'rb')  #以二进制读取模式打开pickle文件
    s = f.read()    #读取文件内容
    data = pickle.loads(s)
    # print(data)
    ...
    print(len(pre_label))
    print(len(real_label))  #打印预测标签和真实标签的数量
    heatmap = np.zeros((4, 4))  #初始化一个 4x4 的矩阵，用于存储热图数据
    for r, p in zip(real_label, pre_label):
        heatmap[r][p] = heatmap[r][p] + 1
    # heatmap = preprocessing.scale(heatmap)
    print(heatmap)
    result = []     #初始化一个列表，用于存储每一行的归一化结果
    for row in heatmap:
        print(row)
        row = row / np.sum(row)
        result.append(row)
...
#生成热图，热图中的每个单元格表示一个真实类别被预测为另一个类别的相对频率，颜色越浅表示相对频率越高
